...
# A script to update logger calls across the project
# This script uses the Node.js script we created earlier to update the files

echo "Starting logger migration script..."

# Find files with logger calls
FILES=$(grep -r --include="*.ts" --include="*.tsx" "logger\." src/ | grep -v "logger.ts" | grep -v "errorHandler.ts" | cut -d: -f1 | sort | uniq)

# Make the update-loggers directory if it doesn't exist
mkdir -p update-loggers

# Create a temporary package.json for our script
cat > update-loggers/package.json << EOF
{
  "name": "update-loggers",
  "version": "1.0.0",
  "description": "Script to update logger calls",
  "main": "index.js",
  "scripts": {
    "start": "node index.js"
  },
...
cat > update-loggers/index.js << EOF
const fs = require('fs');
...
# Process each file
echo "$FILES" | xargs node update-loggers/index.js

echo "Logger migration complete. Run TypeScript check to verify changes:"
echo "npx tsc --noEmit" 