...
import type { GameData } from 'chessTypes'
...
export default function AvailableGames() {
  ...
  const handleJoinGame = async (gameId: string) => {
    try {
      ...
      if (gameData.player1Id && gameData.player1Id === auth.currentUser.uid) {
      ...
      const userData = userSnap.data() as { balance?: number }
      const currentBalance = userData.balance || 0

      if (gameData.wager === undefined || currentBalance < gameData.wager) {
            ...
            <div key={game.id} className="bg-white p-4 rounded-lg shadow">
              ...
              <div className="mb-4">
                <p className="text-sm text-gray-600">
                  Created by: {game.player1Id && auth.currentUser && game.player1Id === auth.currentUser.uid ? "You" : "Another player"}
                </p>
              </div>
              <button
                onClick={() => game.id && handleJoinGame(game.id)}
